...
subplot(2,1,i_cam-1);
h1  = plot(x, mOSet, 'r', 'LineWidth', 1); hold on;
h2  = plot(x, mOFast, 'b', 'LineWidth', 1);

patchX      = [x,flip(x)];
h3  = patch(patchX, [mOSet-sOset; flip(mOSet+sOset)]', 'red', 'EdgeColor','red', 'FaceAlpha',alpha1, 'EdgeAlpha',alpha1); 
h4  = patch(patchX, [mOFast-sOFast; flip(mOFast+sOFast)]', 'blue', 'EdgeColor','blue', 'FaceAlpha',alpha1, 'EdgeAlpha',alpha1);
...
subplot(2,1,i_cam);
set(gca,'visible','off')
legend([h1, h2, h3, h4],{'Mean (Ours)', 'Mean (FastSLAM)', 'Standard deviation (Ours)', 'Standard deviation (FastSLAM)'}, 'Interpreter','latex', 'NumColumns', 1, 'FontSize', fontSize);
